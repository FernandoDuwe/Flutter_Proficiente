...
import 'package:chat/widgets/imageSelector.dart';
...
class _MessageAreaState extends State<MessageArea> {
  ...
  void Internal_SendMessage() {
    MessageRepository.addMessage(MessageModel.textMessage(
        Provider.of<UserStore>(context, listen: false).currentUserName,
        widget.destinyUser.userName,
        this._message));

    this.messageController.text = "";
  }

  void Internal_SendPhoto() {
    showBottomSheet(
      context: context,
      builder: (context) => ImageSelector(destinyUser: widget.destinyUser),
    );
  }
    ...
    return BottomAppBar(
      child: Row(
        children: [
          ...
          IconButton(
              onPressed: this.Internal_SendPhoto,
              icon: Icon(Icons.photo_camera)),
          IconButton(
              onPressed: (this.canSend ? this.Internal_SendMessage : null),
              icon: Icon(
                Icons.send,
                color: (this.canSend ? Colors.green : Colors.grey),
              ))
        ],
      ),
    );
